...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        #     if not cursor.entrance.is_hive:
        #         cursor = cursor.entrance
        # if self.upper_bound:
        #     while self.place_counter < self.upper_bound and not cursor.entrance.is_hive and cursor.bees == []:
        #         cursor = cursor.entrance
        #         self.place_counter += 1
        # else:
        #     while not cursor.entrance.is_hive and cursor.bees == []:
        #         cursor = cursor.entrance
        # if cursor.bees != []:
        #     return random_bee(cursor.bees)
        # return None 
        # # REIMPLEMENT THIS LMAO
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0:
            ...
            chosen_bee = random_bee(self.place.bees)
            chosen_health = chosen_bee.health
            chosen_bee.reduce_health(chosen_health)
            self.turns_to_chew = self.chewing_turns

            # chosen_bee.health = 1
            # chosen_bee.reduce_health(1)
            # random_bee(self.place.bees).health = 0
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.is_waterproof:
            non_waterproof_health = insect.health
            insect.reduce_health(non_waterproof_health)
        # END Problem 10
